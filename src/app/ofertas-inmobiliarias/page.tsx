...
// Mapeos de regiones y URLs
const REGION_MAP: Record<string, { tt: string; pi: string; label: string }> = {
...
function OffersContent() {
  ...
  const [propertyType, setPropertyType] = useState("ambos"); // 'casa', 'departamento', 'ambos'
  ...
  const generateLinks = () => {
    ...
    // 1. URL TOC TOC
    const ttState = newProperty ? "&estado=1" : "&estado=0";
    const toctocUrl = `https://www.toctoc.com/resultados/mapa/compra/departamento-casa/${tt}/?moneda=1&precioDesde=25000000&precioHasta=${maxPriceParam}&dormitoriosDesde=${bedrooms}&banosDesde=1${ttState}&disponibilidadEntrega=&numeroDeDiasTocToc=0&superficieDesdeUtil=0&superficieHastaUtil=0&superficieDesdeConstruida=0&superficieHastaConstruida=0&superficieDesdeTerraza=0&superficieHastaTerraza=0&superficieDesdeTerreno=0&superficieHastaTerreno=0&ordenarPor=0&pagina=1&paginaInterna=1&zoom=15&idZonaHomogenea=0&atributos=&texto=${text}&viewport=${viewport}&idPoligono=${polygon}&publicador=0&temporalidad=0`;

    // 2. URL Portal Inmobiliario
    const projectPath = newProperty ? "/proyectos" : "";
    const portalDeptoUrl = `https://www.portalinmobiliario.com/venta/departamento${projectPath}/${pi}/_DisplayType_M_PriceRange_30000000CLP-${maxPriceParam}CLP`;
    const portalCasaUrl = `https://www.portalinmobiliario.com/venta/casa${projectPath}/${pi}/_DisplayType_M_PriceRange_25000000CLP-${maxPriceParam}CLP_BEDROOMS_${bedrooms}-*`;

    // 3. URL Enlace Inmobiliario
    let enlaceUrl = "";
    if (newProperty && ENLACE_MAP[region]) {
      let subsidySegment = "";
      ...
      if (subsidySegment) {
        ...
        enlaceUrl = `https://www.enlaceinmobiliario.cl/${base}/propiedades/${propTypeSegment}/SD0-SH0/UFD0-UFH${uf}/${bedroomSlug}/${subsidySegment}/banos/entrega/disponibilidad/`;
      }
    }

    // 4. URL de Integración DS19
    let ds19Url = "";
    if (ENLACE_MAP[region]) {
      const base = ENLACE_MAP[region];
      const propTypeSegment = propertyType === "ambos" ? "todas" : propertyType;
      ds19Url = `https://www.enlaceinmobiliario.cl/${base}/propiedades/${propTypeSegment}/SD0-SH0/UFD0-UFH2200/${bedroomSlug}/subsidio-ds19/banos/entrega/disponibilidad/`;
    }
    ...
    <div className="space-y-6">
      ...
      <section className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
        <form onSubmit={handleGenerate} className="space-y-5">
          ...
          <label className="flex items-center gap-2 text-sm font-bold text-slate-700 cursor-pointer py-2 border-t border-slate-100">
            <input 
              type="checkbox" 
              className="w-4 h-4 text-[#6b9ac4] rounded" 
              checked={newProperty} 
              onChange={(e) => setNewProperty(e.target.checked)} 
            /> 
            Buscar solo proyectos inmobiliarios nuevos (en blanco/verde/entrega)
          </label>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                ...
                <span className="text-xs text-slate-500 block">Buscar {propertyType === 'ambos' ? 'propiedades' : propertyType === 'casa' ? 'casas' : 'departamentos'} nuevos/usados con mapa interactivo.</span>
                <span className="text-xs text-blue-600 font-semibold mt-3 block group-hover:underline">Abrir en TocToc →</span>
              </a>

              {links.enlaceUrl && (
                <a 
                  href={links.enlaceUrl} 
                  target="_blank" 
                  rel="noopener noreferrer" 
                ...
                >
                  ...
                  <span className="text-xs text-slate-500 block">Proyectos nuevos filtrados con financiamiento de subsidio {originParam?.toUpperCase()}.</span>

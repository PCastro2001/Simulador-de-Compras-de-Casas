// src/app/percentil/ds1t3/page.tsx
"use client";
...
// 1. DICCIONARIO DE PUNTAJES DE CORTE (DS1 TRAMO 3 - PRIMER LLAMADO 2025)
const PUNTAJES_CORTE_T3: Record<string, number> = {
  arica: 109.26,
  tarapaca: 63.30,
  antofagasta: 114.70,
  atacama: 65.21,
...
export default function AhorroSeguroDS1T3Page() {
  ...
  const handleCalculate = (e: React.FormEvent) => {
    ...
    // C. CÁLCULO DE LA BRECHA Y EL AHORRO EXTRA (FÓRMULA EXCLUSIVA TRAMO 3)
    ...
    if (puntosFaltantes > 0) {
      ...
      let maxPuntosFase1 = 60 * 2; // 120 pts
      ...
      if (puntosPorCubrir > 0) {
          let maxPuntosFase2 = 50 * 1; // 50 pts
      ...
      if (puntosPorCubrir > 0) {
          let maxPuntosFase3 = 50 * 0.5; // 25 pts
          if (puntosPorCubrir <= maxPuntosFase3) {
              ufExtraNecesarias += puntosPorCubrir / 0.5;
              puntosPorCubrir = 0;
          } else {
              ufExtraNecesarias += 50;
    ...
    const ahorroMetaUF = AHORRO_MINIMO_T3 + ufExtraNecesarias;
    ...
    <div className="bg-slate-100 min-h-screen text-slate-800 font-sans pb-12">
      <header className="bg-gradient-to-r from-[#6b9ac4] to-[#87c0a3] text-white px-6 py-10 border-b-4 border-white shadow-sm">
        <div className="max-w-3xl mx-auto flex justify-between items-center">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold tracking-tight">Ahorro Seguro: DS1 Tramo 3</h1>
          ...
          <section id="resultados-ahorro" className="space-y-4 animate-fade-in">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
              <div className="flex flex-col md:flex-row justify-between gap-6 mb-6">
                <div>
                  <h3 className="text-sm font-bold text-slate-500 uppercase tracking-widest mb-1">Análisis de Puntaje (Tramo 3)</h3>
              ...
              <div className="bg-blue-50 border-2 border-blue-200 p-6 rounded-xl text-center">
                ...
                {results.puntosFaltantes === 0 && (
                  <p className="text-xs text-slate-500 mt-4">Como tu puntaje base ya supera el corte, solo se te exige el mínimo legal establecido para el Tramo 3 ({AHORRO_MINIMO_T3} UF).</p>
                )}
              </div>
            </div>
          </section>
